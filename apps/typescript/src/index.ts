...
const main = async (): Promise<void> => {
  ...
  /**
   * Wrapper promise for the producer side-effect.
   *
   * Calling `producer.produce()` immediately kicks off the asynchronous
   * work (a `setTimeout` inside `Producer.produce`). The method itself
   * returns `void`, so we wrap the call in a `Promise<void>` that resolves
   * right away. This lets us include a "producer task" in `Promise.all`
   * so the intent ("run producer and consumer in parallel") is explicit
   * and future producer logic that might become asynchronous will already
   * have the correct structure. The actual delay / async completion is
   * represented by the original `promise` the `Consumer` awaits.
   */
